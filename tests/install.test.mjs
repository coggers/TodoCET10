...
// ---- 1. iOS: button shows, opens Share-based instructions ----
{
  ...
  ok((await p.locator('#install').getAttribute('aria-label')).includes('Add to Home Screen'), 'button labelled clearly (icon-only, label on aria-label)');
...
// ---- 3. already installed => button hidden ----
{
  ...
  ok((await p.locator('#install').getAttribute('aria-label')).includes('Share'), 'iOS standalone: labelled Share');
